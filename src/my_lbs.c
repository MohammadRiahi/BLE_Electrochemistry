...
static uint16_t read_adc_channel(uint8_t channel) {
	// Update sequence for the specified channel
...
int my_lbs_init(struct my_lbs_cb *callbacks)
{
	...
	if (gpio_dev) {
		...
		gpio_pin_configure(gpio_dev, LED2_PIN, GPIO_OUTPUT_ACTIVE); // LED2 on P0.24
		gpio_pin_set(gpio_dev, LED2_PIN, 0); // Turn off LED2 initially
	}
	
	if (gpio1_dev) {
		gpio_pin_configure(gpio1_dev, LED1_PIN, GPIO_OUTPUT_ACTIVE); // LED1 on P1.08  
		gpio_pin_set(gpio1_dev, LED1_PIN, 0); // Turn off LED1 initially
	}
